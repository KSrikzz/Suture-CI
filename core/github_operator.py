...
class GitOperator:
    def __init__(self, token, repo_name):
        if not token:
            raise ValueError(
                "CRITICAL: Environment variable 'GITHUB_TOKEN' is missing.\n"
                "Please configure 'GITHUB_TOKEN' in your environment or .env file."
            )
        if not repo_name:
            raise ValueError(
                "CRITICAL: Environment variable 'GITHUB_REPO' is missing.\n"
                "Please configure 'GITHUB_REPO' in your environment or .env file."
            )
        self.g = Github(token)
    ...
    def create_fix_pr(self, branch_name, file_path, new_content, commit_message, pr_title, pr_body):
        try:
            ...
            try:
                pr.as_issue().add_to_labels("autofix", "ai-generated")
